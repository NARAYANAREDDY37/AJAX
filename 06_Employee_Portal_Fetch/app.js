...
addEmployeeForm.addEventListener('submit', (e) => {
	...
	$('#add-employee-modal').modal('hide'); //to automatically close the modal when you submit the form (jquery snnipet)
	...
	//now sending this dynamic object to the AJAX post request to store the data
...
// clearing the form fields after adding employee
...
tableBody.addEventListener('click', (e) => {
	...
	if (targetElement.classList.contains('delete')) {
		//getting the specific ID of that button when clicked
		let selectedId =
			targetElement.parentElement.parentElement.firstElementChild.innerHTML;
	...
	if (targetElement.classList.contains('update')) {
		...
		fetch.get(url).then((data) => {
			...
			//now we got the object of selected employee and we need to populate to the modal
...
let populateUpdateModal = (selectedEmployee) => {
	...
	document.querySelector('#update-ip-address').value =
		selectedEmployee.ip_address;
	$('#update-employee-modal').modal('show'); //to show the modal when you click on update
};
...
updateEmployeeForm.addEventListener('submit', (e) => {
	...
	let fetch = new BrainHttp();
	fetch.put(url, employee).then((data) => {
		console.log(data);
		fetchAllEmployees();
	});
});
